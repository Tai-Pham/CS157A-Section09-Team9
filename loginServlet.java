...
public class loginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {	
            ...
            ResultSet rs = stmt.executeQuery("SELECT `Account ID`, Username, Password FROM cs157a.`account` WHERE Username LIKE '%"+username+"%' AND Password LIKE '%"+password+"%'");
            while (rs.next()) {
            ...
            if(accountID == 0) {
            	...
            	if(username.equals(dbUsername) && password.equals(dbPassword))
            	{
		            ...
		            switch(type) {
		            	case "Client":
		            		statement = "SELECT * FROM cs157a.`client` WHERE `Account ID` = "+ accountID +" LIMIT 1";
		            		break;	
		            	case "Tester":
		            		//Need to make sure the account is Verified before the tester can log in
		            		statement = "SELECT * FROM cs157a.`tester` WHERE `Account ID` = "+ accountID +" AND `Verified` = 1 LIMIT 1";
		            		break;		
		            	case "Admin":
		            		statement = "SELECT * FROM cs157a.`admin` WHERE `Account ID` = "+ accountID +" LIMIT 1";
		            		break;
		            }
